...
import { DomainResponse } from '@/modules/domain/domain.module';
...
const DomainRemoveModal: React.FC<DomainProps> = ({
    domain,
    onReload
}) => {
    ...
    const removeDomain = () => {
        if (!domain || !domain.id) {
            setToastInfo({
                show: true,
                message: `Xóa DomainID lỗi! Vui lòng kiểm tra lại domain!`,
                variant: ToastVariant.ERROR
            });
            return;
        }
        DomainService.deleteDomainByIdRequest(domain?.id)
            .then(() => {
                setToastInfo({
                    show: true,
                    message: `Xóa thành công domain ${domain.name}!`,
                ...
                setToastInfo({
                    show: true,
                    message: `Xóa thất bại domain ${domain.name}! Lỗi ${error?.message || 'Không xác định'}`,
                    variant: ToastVariant.ERROR
                });
            })
    }
    ...
    return (
        <>
            <Button className="bg-red-400 hover:bg-red-500" size="sm" variant="primary"
                startIcon={<TrashIcon />}
                onClick={openModal}
            />
            <Modal
            ...
            >
                <div className="text-center">
                    ...
                    <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90 sm:text-title-sm">
                        Thông báo xóa Domain
                    </h4>
                    <p className="text-sm leading-6 text-gray-500 dark:text-gray-400">
                        Bạn có chắc chắn muốn xóa domain <span className="font-semibold">{domain?.name}</span> không?
                        Việc này sẽ xóa vĩnh viễn tất cả dữ liệu liên quan đến domain này.
                    </p>

                    <div className="flex items-center justify-center w-full gap-3 mt-7">
                        <Button className="flex justify-center w-full px-4 py-3 text-sm font-medium text-white rounded-lg bg-error-500 shadow-theme-xs hover:bg-error-600 sm:w-auto" size="sm" variant="primary"
